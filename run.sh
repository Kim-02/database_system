...
MMS=(128 256 512 1024)
OMPS=(2 4 8)

for mm in "${MMS[@]}"; do
  for omp in "${OMPS[@]}"; do
    # S: B_L = B_R
    avg_tc "S1" "$omp" "$mm" 1024  1024
    avg_tc "S2" "$omp" "$mm" 2048  2048
    avg_tc "S3" "$omp" "$mm" 4096  4096
    avg_tc "S4" "$omp" "$mm" 8192  8192
    avg_tc "S5" "$omp" "$mm" 16384 16384

    # R: LEFT 고정(4096), RIGHT만 변화
    avg_tc "R1" "$omp" "$mm" 4096  1024
    avg_tc "R2" "$omp" "$mm" 4096  2048
    avg_tc "R3" "$omp" "$mm" 4096  4096
    avg_tc "R4" "$omp" "$mm" 4096  8192
    avg_tc "R5" "$omp" "$mm" 4096  16384

    # L: RIGHT 고정(4096), LEFT만 변화
    avg_tc "L1" "$omp" "$mm" 1024  4096
    avg_tc "L2" "$omp" "$mm" 2048  4096
    avg_tc "L3" "$omp" "$mm" 4096  4096
    avg_tc "L4" "$omp" "$mm" 8192  4096
    avg_tc "L5" "$omp" "$mm" 16384 4096
  done
done